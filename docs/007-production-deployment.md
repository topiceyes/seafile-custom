...
**恒判 https 加固（2026-09-22，tag `12.0.14-dingtalk.9.68238d48` = `latest`，digest
`sha256:7bc4f11b…`；起因：生产第二次撞登录 403，形态是「代理把转发头写坏」）：**

| 项 | 实测 |
|---|---|
| 反代彩排（§7.2）全量重跑 | ✅ 34 条断言全绿（新镜像，amd64/Rosetta）：登录 302、上传下载逐字节一致、升级无操作、回滚覆盖 |
| **P2 探针翻义验证** | ✅ `X-Forwarded-Proto: http` → **302**（上一版镜像是 403）：容器不再采信上游值，代理乱发头免疫 |
| 根因链 | 代理把它「自己到后端这段」的协议写进头（`http`）或多级拼成 `"https, http"`；Django 要求头值与 `SECURE_PROXY_SSL_HEADER` 配置**逐字相等**，任何偏差 → `is_secure()` 为假 → 403。首起事故（09-21）是**根本没设** `SECURE_PROXY_SSL_HEADER`，这次是**设了但头值不对**——同症不同因，第二次才把「采信上游」这半也拆掉 |
