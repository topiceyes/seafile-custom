...
# 渲染结果写入 seafile-data/seafile/conf/seahub_settings.py（该目录不入库）。

SECRET_KEY = "__SEAHUB_SECRET_KEY__"

# 站点地址。必须与「容器环境变量 SEAFILE_SERVER_HOSTNAME / SEAFILE_SERVER_PROTOCOL」
# （见 seafile-server.yml，会覆盖本行，见 seahub/settings.py:1172）以及
